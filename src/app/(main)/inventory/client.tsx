...
/** Gets the display color string for a roll (resolves Blackout number → name). */
function rollDisplayColor(roll: Roll): string {
  if (isBlackoutProduct(roll.category.name)) {
    return getBlackoutColorName(roll.product.code, roll.product.color);
  }
  return roll.product.color;
}

/** Syncs filter state to URL without triggering Next.js navigation. */
function updateUrl(filters: Record<string, string>) {
  if (typeof window === 'undefined') return;
  const url = new URL(window.location.href);
  const filterKeys = ['q', 'cat', 'status', 'color', 'minM', 'maxM', 'loc'];
...
export default function InventoryClient({
  ...
  // Display consecutivo numbers (stable across filters/pages)
  const displayNumbers = useMemo(() => {
    const OFFSETS_MAP: Record<string, number> = { Velo: 1001, Blackout: 1201 };
    ...
    for (const r of sorted) {
      ...
      const start = OFFSETS_MAP[cat] ?? DEFAULT_OFFSET;
  ...
  // Available filter options (derived from full rolls list)
  ...
  const filtered = useMemo(() => {
    ...
    return tabRolls.filter(r => {
      ...
      const consecutivo = String(displayNumbers.get(r.id) ?? '');
      ...
      const matchSearch = !q ||
        ...
        (r.labelNumber != null ? String(r.labelNumber).includes(q) : false) ||
        consecutivo.includes(q) ||
  ...
  }, [tabRolls, search, categoryFilter, statusFilter, colorFilter, minMeters, maxMeters, locationFilter, displayNumbers]);
  ...
  // Sync filters to URL (debounced, no Next.js navigation)
  useEffect(() => {
  ...
  async function handleExit() {
    ...
    try {
      ...
      if (!res.ok) {
        setToast({ message: data.error ?? 'Error al registrar salida', type: 'error' }); return;
      }
  ...
  async function handleEntry(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/inventory/entry', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          rollNumber: entryRollNumber, productId: parseInt(entryProductId),
          lotId: parseInt(entryLotId), initialMeters: parseFloat(entryMeters),
          location: entryLocation, barcode: entryBarcode || null,
        }),
      });
      const data = await res.json();
      if (!res.ok) {
        setToast({ message: data.error ?? 'Error al registrar entrada', type: 'error' }); return;
      }
  ...
  // Consecutivo | N° Etiqueta | Referencia | Producto | Color | Metros | Estado | Ubicación | [Precio B2B] | Acciones
      ...
      <div className="hidden md:flex flex-wrap gap-2 mb-3 items-center">
        ...
        <div className="relative flex-1 min-w-48">
          ...
          <input
            ...
            placeholder="Buscar por referencia, producto, N° etiqueta, color, ubicación..."
      ...
      <div className="flex md:hidden gap-2 mb-3">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-2 border rounded text-sm transition-colors ${
            activeFilterCount > 0 ? 'border-gray-900 bg-gray-900 text-white' : 'border-[#E5E5E5] bg-white text-gray-700'
          }`}
        >
          Filtros
          {activeFilterCount > 0 && (
            <span className="bg-white text-gray-900 text-xs font-bold w-4 h-4 flex items-center justify-center rounded-full">
              {activeFilterCount}
            </span>
          )}
        ...
        <div className="md:hidden bg-white border border-[#E5E5E5] rounded-lg p-4 mb-3 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input type="number" min="0" value={minMeters} onChange={e => setMinMeters(e.target.value)} placeholder="0" className="w-full border border-[#E5E5E5] rounded px-3 py-2.5 text-sm focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Metros máx.</label>
              <input type="number" min="0" value={maxMeters} onChange={e => setMaxMeters(e.target.value)} placeholder="∞" className="w-full border border-[#E5E5E5] rounded px-3 py-2.5 text-sm focus:outline-none" />
            </div>
          </div>
          ...
          <table className="w-full text-sm min-w-[820px]">
            <thead>
              <tr className="border-b border-[#E5E5E5] text-xs text-gray-500 uppercase tracking-wide bg-gray-50">
                ...
                <th className="px-4 py-3 text-left">N° Etiqueta</th>
                  ...
                  return (
                    <tr key={roll.id} className="border-b border-[#F5F5F5] hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 font-mono">
                        <div className="font-semibold text-gray-800 text-sm">{displayNumbers.get(roll.id) ?? roll.id}</div>
                        <div className="text-[10px] text-gray-400 mt-0.5">{roll.rollNumber}</div>
                      </td>

                      {/* N° Etiqueta */}
                      <td className="px-4 py-3 font-mono text-sm text-gray-700">
                        {roll.labelNumber != null ? (
                          <span className="font-semibold">{roll.labelNumber}</span>
                        ) : (
                      ...
                      <td className="px-4 py-3">
                        <div className="font-medium text-gray-900 text-sm">{roll.product.name}</div>
                        <div className="text-xs text-gray-400">{roll.product.width} cm</div>
                      </td>

                      {/* Color */}
                      <td className="px-4 py-3 text-sm text-gray-600">{displayColor}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          <Link
                            ...
                            className="text-xs text-gray-400 hover:text-gray-700 border border-[#E5E5E5] px-2 py-1.5 rounded hover:bg-gray-50 transition-colors"
                          >
                            ⊙
                          </Link>
                        </div>
              ...
              {exitStep === 'product' && (
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3 pt-1">
                    <button type="button" onClick={closeExit} className="flex-1 border border-[#E5E5E5] rounded px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">Cancelar</button>
                    <button type="button" onClick={handleProductNext} disabled={!exitProductId} className="flex-1 bg-[#0A0A0A] text-white rounded px-4 py-2 text-sm font-medium hover:bg-[#1A1A1A] disabled:opacity-40">Siguiente →</button>
                  </div>
                </div>
              )}
              ...
              {exitStep === 'remnant-check' && (
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    <button type="button" onClick={() => setExitStep('product')} className="flex-1 border border-[#E5E5E5] rounded px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">← Volver</button>
                    <button type="button" onClick={() => { const s = remnantWarning[0]; setExitRoll(s); setExitPrevStep('remnant-check'); setExitStep('confirm'); }} className="flex-1 bg-amber-500 text-white rounded px-4 py-2 text-sm font-medium hover:bg-amber-600">Usar remanente</button>
                    <button type="button" onClick={() => setExitStep('roll')} className="flex-1 bg-[#0A0A0A] text-white rounded px-4 py-2 text-sm font-medium hover:bg-[#1A1A1A]">Continuar igual</button>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {rollsForProduct.map(r => (
                      <button key={r.id} type="button" onClick={() => { setExitRoll(r); setExitPrevStep('roll'); setExitStep('confirm'); }}
                        className={`w-full flex items-center justify-between border rounded-lg px-4 py-3 text-sm hover:border-gray-400 transition-colors ${exitRoll?.id === r.id ? 'border-gray-900 bg-gray-50' : 'border-[#E5E5E5]'}`}>
                        <div className="text-left">
                          <div className="font-mono text-xs text-gray-600 font-semibold">{r.rollNumber}</div>
                          {r.labelNumber != null && <div className="text-xs text-gray-400">Etiq. {r.labelNumber}</div>}
                          <div className="text-xs text-gray-400 mt-0.5">{r.location}</div>
                        </div>
                        <div className="text-right">
                          <div className={`font-semibold text-sm ${r.isRemnant ? 'text-amber-600' : 'text-green-700'}`}>{r.currentMeters} m</div>
              ...
              {exitStep === 'confirm' && exitRoll && (
                <div className="space-y-4">
                  <div className="bg-gray-50 rounded-lg p-4 grid grid-cols-2 gap-3 text-sm">
                    <div><span className="text-gray-400 block text-xs">Referencia</span><span className="font-mono font-semibold text-xs">{exitRoll.product.code}</span></div>
                    <div><span className="text-gray-400 block text-xs">Producto</span><span className="font-medium">{exitRoll.product.name}</span></div>
                    <div><span className="text-gray-400 block text-xs">Color</span><span>{rollDisplayColor(exitRoll)}</span></div>
                    <div><span className="text-gray-400 block text-xs">Rollo proveedor</span><span className="font-mono font-medium text-xs">{exitRoll.rollNumber}</span></div>
                    {exitRoll.labelNumber != null && <div><span className="text-gray-400 block text-xs">N° Etiqueta</span><span className="font-semibold">{exitRoll.labelNumber}</span></div>}
                    ...
                    <div className="flex gap-2">
                      {(['EXIT_PARTIAL', 'EXIT_FULL'] as const).map(t => (
                        <button key={t} type="button" onClick={() => setExitType(t)}
                          className={`flex-1 border rounded-lg px-3 py-3 text-sm text-left transition-colors ${exitType === t ? 'border-gray-900 bg-gray-50' : 'border-[#E5E5E5] hover:border-gray-400'}`}>
                          <div className="font-semibold text-gray-900 text-xs mb-0.5">{t === 'EXIT_PARTIAL' ? 'Salida parcial' : 'Salida total'}</div>
                          <div className="text-xs text-gray-400">{t === 'EXIT_PARTIAL' ? 'Corte por metros' : `Vender completo (${exitRoll.currentMeters}m)`}</div>
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  {exitType === 'EXIT_PARTIAL' && (
                    <div>
                      ...
                      <input type="number" step="0.1" min="0.1" max={exitRoll.currentMeters} value={exitMeters} onChange={e => setExitMeters(e.target.value)} autoFocus
                        className="w-full border border-[#E5E5E5] rounded px-3 py-2 text-sm focus:outline-none focus:border-gray-400" placeholder={`Máx. ${exitRoll.currentMeters} m`} />
                  ...
                  {/* Client — combobox con creación inline */}
                  <div>
                    <label className="block text-xs font-medium text-gray-700 uppercase tracking-wide mb-1.5">
                    ...
                    <ClientCombobox
                      ...
                      onClientCreated={newC =>
                        setClientsList(prev =>
                          [...prev, newC].sort((a, b) => a.name.localeCompare(b.name))
                        )
                      }
                  ...
                  <div>
                    ...
                    <textarea value={exitNotes} onChange={e => setExitNotes(e.target.value)} rows={2}
                      className="w-full border border-[#E5E5E5] rounded px-3 py-2 text-sm resize-none focus:outline-none focus:border-gray-400" placeholder="Observaciones opcionales..." />
                  </div>

                  <div className="flex gap-3 pt-1">
                    <button type="button" onClick={() => setExitStep(exitPrevStep)} className="border border-[#E5E5E5] rounded px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">← Volver</button>
                    <button type="button" onClick={handleExit} disabled={exitLoading || !exitClient || (exitType === 'EXIT_PARTIAL' && !exitMeters)}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  ...
                  <input type="text" value={entryRollNumber} onChange={e => setEntryRollNumber(e.target.value)} required
                    className="w-full border border-[#E5E5E5] rounded px-3 py-2 text-sm focus:outline-none focus:border-gray-400" placeholder="Ej. CH-250210-084" />
                </div>
                <div className="col-span-2">
                  <label className="block text-xs font-medium text-gray-700 uppercase tracking-wide mb-1">
                  ...
                  <select value={entryProductId} onChange={e => setEntryProductId(e.target.value)} required
                    ...
                    {products.map(p => (
                      <option key={p.id} value={p.id}>{p.code} — {p.name} {p.color} {p.width}cm</option>
                    ))}
                  </select>
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="number" step="0.1" min="1" value={entryMeters} onChange={e => setEntryMeters(e.target.value)} required
                    className="w-full border border-[#E5E5E5] rounded px-3 py-2 text-sm focus:outline-none focus:border-gray-400" placeholder="150" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 uppercase tracking-wide mb-1">
                  ...
                  <input type="text" value={entryLocation} onChange={e => setEntryLocation(e.target.value)} required
                    className="w-full border border-[#E5E5E5] rounded px-3 py-2 text-sm focus:outline-none focus:border-gray-400" placeholder="A-01" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-700 uppercase tracking-wide mb-1">
                  ...
                  <input type="text" value={entryBarcode} onChange={e => setEntryBarcode(e.target.value)}
                    className="w-full border border-[#E5E5E5] rounded px-3 py-2 text-sm focus:outline-none focus:border-gray-400" placeholder="DISA-VP-001-01" />
                </div>
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowEntry(false)} className="flex-1 border border-[#E5E5E5] rounded px-4 py-2 text-sm text-gray-600 hover:bg-gray-50">Cancelar</button>
                <button type="submit" disabled={entryLoading} className="flex-1 bg-[#0A0A0A] text-white rounded px-4 py-2 text-sm font-medium hover:bg-[#1A1A1A] disabled:opacity-50">
